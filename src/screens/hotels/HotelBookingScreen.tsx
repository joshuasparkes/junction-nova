...
type HotelBookingScreenRouteProp = RouteProp<
  HotelStackParamList,
  'HotelBooking'
>;
type HotelBookingScreenNavigationProp = StackNavigationProp<
  HotelStackParamList,
  'HotelBooking'
>;
...
const HotelBookingScreen = () => {
  ...
  const handleConfirmBooking = () => {
    if (
      ...
      Alert.alert(
        'Validation Error',
        'Please fill in all guest and booking details.',
      );
      return;
    }
    navigation.navigate('BookingSuccess', {
      message: 'Hotel Booking Confirmed!',
      bookedItemName: `${hotel.name} - ${hotel.location}`,
      details: [
  ...
  return (
    <ScrollView
      ...
      <Text style={styles.screenTitle}>Book Hotel</Text>

      <View style={styles.itemDetailsCard}>
        <Text style={styles.detailTitle}>{hotel.name}</Text>
        <Text style={styles.detailText}>Location: {hotel.location}</Text>
        <Text style={styles.detailText}>Rating: {hotel.rating} / 5</Text>
        <Text style={styles.detailText}>Amenities: {hotel.amenities}</Text>
